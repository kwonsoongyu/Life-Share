...
df_1=df.loc[:,['기준년월','월소득평균금액','대출평균잔액','카드이용평균금액']]
...
#전처리
df_2=df_1.groupby(by='기준년월').mean().reset_index()
df_2['대출평균잔액']=df_2['대출평균잔액']//12
df_2['월소득평균금액']=df_2['월소득평균금액'].astype('int')
df_2['대출평균잔액']=df_2['대출평균잔액'].astype('int')
df_2['카드이용평균금액']=df_2['카드이용평균금액'].astype('int')

#그래프 만들기
fig=make_subplots(rows=1, cols=1, shared_xaxes=True)
fig.add_trace(go.Scatter(x=df_2['기준년월'],y=df_2['월소득평균금액'],mode='lines+markers',name='월소득평균금액'))
fig.add_trace(go.Scatter(x=df_2['기준년월'],y=df_2['대출평균잔액'],mode='lines+markers',name='대출평균잔액'))
fig.add_trace(go.Scatter(x=df_2['기준년월'],y=df_2['카드이용평균금액'],mode='lines+markers',name='카드이용평균금액'))
              
#그래프 레이아웃 설정
fig.update_layout(title='<b>시간에 따른 월소득금액, 카드이용금액, 대출평균잔액 비교<b>',
                  xaxis={'title':{'text':'기준년월'}},
                  yaxis={'title':{'text':'금액'}})
plotly.offline.plot(fig,filename='3.시간에 따른 월소득금액, 대출평균잔액, 카드이용평균금액 비교.html')
