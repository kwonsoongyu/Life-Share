...
df_1 = df.loc[:, ["기준년월", "월소득평균금액", "보유승용차평균가격"]]
...
df_1['기준년월'] = df_1['기준년월'].astype('str')
df_2 = round(df_1.groupby(['기준년월'], as_index = False).mean(), 3)
...
fig.add_trace(go.Bar(
    ...
    name = '월소득평균금액',
    orientation='h',
    marker = dict(color='rgb(242, 209, 209)', line = dict(color='rgb(242, 209, 209)'))
))
fig.add_trace(go.Bar(
    y = df_2['기준년월'],
    x = df_2['보유승용차평균가격'],
    name = '보유승용차평균가격',
    orientation='h',
    marker = dict(color='rgb(202, 184, 255)', line = dict(color='rgb(202, 184, 255)'))
))

#그래프 레이아웃 및 출력
fig.update_layout(title='<b>시간에 따른 월소득평균금액 보유승용차평균가격 비교</b>', barmode='stack',
                  xaxis={'title': {'text': '가격(금액)'}},
                 yaxis={'title': {'text': '기준년월'}})

plotly.offline.plot(fig, filename = '3-1.html')
