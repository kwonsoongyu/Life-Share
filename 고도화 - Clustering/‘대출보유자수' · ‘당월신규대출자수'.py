...
"8백초과9백만원이하월소득평균금액","9백초과1천만원이하월소득평균금액", "1천만원초과월소득평균금액",
...
#5. clustering 결과 시각화
plt.rcParams["font.family"] = 'NanumGothic'
p=sns.scatterplot(data=df, x="대출보유자수", y="당월신규대출자수", hue='cluster',palette='Set2')
...
df[df['cluster'].isin([-1,2])]

#7. 이상치 제거 후 클러스터링 2차 수행
df = df[-df['cluster'].isin([-1,2])] #row 제거하기
...
plt.rcParams["font.family"] = 'NanumGothic'
p=sns.scatterplot(data=df, x="대출보유자수", y="당월신규대출자수", hue='cluster_fn',palette='Set2')
sns.move_legend(p,"upper right",bbox_to_anchor=(1.15,1.0),title="Clusters",facecolor="white")
sns.set(rc={'axes.facecolor' : '#E5ECF6', 'figure.figsize':(10,8)})
plt.show()

#13. 이상치 제거 후 클러스터링 3차 수행
df = df[-df['cluster_fn'].isin([-1,2])] #row 제거하기
