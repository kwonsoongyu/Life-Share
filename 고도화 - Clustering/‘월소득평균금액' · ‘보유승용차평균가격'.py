...
"월소득평균2백만원이하대상자수","월소득평균2백초과3백만원이하대상자수","월소득평균3백초과4백만원이하대상자수", "월소득평균4백초과5백만원이하대상자수", "월소득평균5백초과6백만원이하대상자수", "월소득평균6백초과7백만원이하대상자수",
"월소득평균7백초과8백만원이하대상자수","월소득평균8백초과9백만원이하대상자수", "월소득평균9백초과1천만원이하대상자수", "월소득평균1천만원초과대상자수"]

#<1차 Clustering>
#1. 표준화
df_labeled=df[['월소득평균금액','보유승용차평균가격']]
scaler=StandardScaler()
df_scaled=scaler.fit_transform(df_labeled)
pd.DataFrame(df_scaled)

#2. 클러스터링
model=DBSCAN(eps=0.22,min_samples=10)
...
#5. clustering 결과 시각화
plt.rcParams["font.family"] = 'NanumGothic'
p=sns.scatterplot(data=df, x="월소득평균금액", y="보유승용차평균가격", hue='cluster',palette='Set2')
...
df[df['cluster'].isin([-1,5])]

#7. 이상치 제거 후 클러스터링 2차 수행
df = df[-df['cluster'].isin([-1,5])] #row 제거하기
...
#9. 클러스터링
model=DBSCAN(eps=0.21,min_samples=10)
clusters=model.fit(df_scaled)
...
plt.rcParams["font.family"] = 'NanumGothic'
p=sns.scatterplot(data=df, x="월소득평균금액", y="보유승용차평균가격", hue='cluster_fn',palette='Set2')
sns.move_legend(p,"upper right",bbox_to_anchor=(1.15,1.0),title="Clusters",facecolor="white")
sns.set(rc={'axes.facecolor' : '#E5ECF6', 'figure.figsize':(10,8)})
plt.show()

#13. 이상치 제거 후 클러스터링 3차 수행
df = df[-df['cluster_fn'].isin([-1,8])] #row 제거하기
